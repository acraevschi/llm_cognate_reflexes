...
from finetune_helper import run_epoch
import json
...
for dir_name in lst_dirs:
    print("Currently fine-tuning for " + dir_name)
    file_path = (
        "ST2022/data/" + dir_name if training else "ST2022/data-surprise/" + dir_name
    )
    ...
    val_dataloader = DataLoader(
        val_data, batch_size=32, shuffle=True, generator=torch.Generator(device=device)
    )
    ...
    model.load_state_dict(
        torch.load("checkpoint_byt5_cognates.pt", map_location=device)
    )
    model.to(device)
    ...
    optim = torch.optim.AdamW(
        model.parameters(), lr=lr, betas=(0.9, 0.98), weight_decay=0.001
    )
    ...
    for n in range(n_epochs):
        ...
        if n > 0:
            if val_loss_lst[-1] > min(val_loss_lst[:-1]):
                ...
                torch.save(
                    model.state_dict(),
                    f"{file_path}/checkpoint_{dir_name}_{prop_path}.pt",
                )
                print(
                    f"Model saved to {file_path}/checkpoint_{dir_name}_{prop_path}.pt"
                )
    print("_" * 100)
