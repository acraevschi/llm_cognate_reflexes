...
for dir_name in lst_dirs:
    file_path = (
        "ST2022/data/" + dir_name + f"/training-{prop_path}.tsv"
        if training
    ...
    for i, head in enumerate(header):
        for n, data_point in enumerate(train_data):
            if (
                ...
                split_cognates_lst = split_cognates(
                    cognate_set, cognate_langs, lang_token_mapping
                )

                for split_cognate in split_cognates_lst:
                    cognate_forms.append(
                        " | ".join(split_cognate) + f" | {lang_token_mapping[head]}"
                    )  # add target language to cognate forms
    ...
    tokenized_inputs = tokenizer(
        cognate_forms, padding=True, truncation=True, return_tensors="pt"
    )
    # Tokenize output sequences
    tokenized_outputs = tokenizer(
        target_form, padding=True, truncation=True, return_tensors="pt"
    )
    ...
    val_tokenized_inputs = tokenizer(
        val_cognate_forms, padding=True, truncation=True, return_tensors="pt"
    )

    # Tokenize validation output sequences
    val_tokenized_outputs = tokenizer(
        val_target_form, padding=True, truncation=True, return_tensors="pt"
    )
